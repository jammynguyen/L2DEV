...
{
  public class SetupTelegramStructureManager :ISetupTelegramsSenderManager
  {
    #region members
    ...
    private readonly SetupTelegramStructureHandler _setupTelegramStructureHandler;
    private readonly SetupTelegramsHandler _setupTelegramsHandler;

    #endregion
    #region ctor

    public SetupTelegramStructureManager(ISetupTelegramSendOffice sendOffice)
    {
      _sendOffice = sendOffice;
      _setupTelegramStructureHandler = new SetupTelegramStructureHandler();
    ...
    public virtual async Task<DataContractBase> ProcessForceSendTelegramSetupAsync(DCTelegramSetupId message)
    {
      ...
      using (PEContext ctx = new PEContext())
      {
        try
        {
          ...
          DCCommonSetupStructure dataContract = _setupTelegramStructureHandler.ConvertTelegramSetupDataToTransferObject(ctx, data, message.TelegramId);
