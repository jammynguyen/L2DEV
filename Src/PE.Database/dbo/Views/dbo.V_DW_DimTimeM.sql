...
AS
     WITH digits(i)
          AS (SELECT 1 AS i
              ...
              SELECT 0 AS Expr1),
          sequence(i)
          AS (SELECT((D1.i + 10 * D2.i) + 100 * D3.i) + 1000 * D4.i AS Expr1
              FROM digits AS D1
          ...
          SELECT CAST('PE_Lite' AS NVARCHAR(50)) AS DataSource, 
                 ...
                 CAST(i + 1 AS BIGINT) AS DimTimeMinKey, 
                 DATEPART(hh, dateVal) AS TimeHours, 
                 DATEPART(mi, dateVal) AS TimeMinutes, 
                 i / 30 % 2 + 1 AS TimeHalfHour, 
                 i / 15 % 4 + 1 AS TimeQuarterHour, 
                 i + 1 AS TimeMinuteOfDay, 
                 i / 30 + 1 AS TimeHalfHourOfDay, 
                 i / 15 + 1 AS TimeQuarterHourOfDay, 
