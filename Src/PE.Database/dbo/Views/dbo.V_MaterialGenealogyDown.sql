CREATE VIEW dbo.V_MaterialGenealogyDown
AS
WITH TREE AS (
...
WHERE        1 = 1
UNION ALL
SELECT        RMI.RawMaterialId MaterialId, RMI.RawMaterialName, ISNULL(RMI.ChildsNo, 0) ChildsNo, RMI.ParentRawMaterialId ParentId, RMI.CuttingSeqNo ParentsCut, TREE.RootInitialLength, 0 IsAncestor, 
...
FROM            dbo.MVHRawMaterials RMI INNER JOIN
                         TREE ON TREE.MaterialId = RMI.ParentRawMaterialId)
    SELECT        [Path], [Level] LevelNo, RootId, MaterialId, ParentId, ChildsNo, IsParent, ParentsCut, RootInitialLength, RMS1.[Length] InitialLength, RMS0.[Length] ActualLength, RawMaterialName, 
     ...
     FROM            TREE INNER JOIN
                              ...
                              dbo.MVHRawMaterialsSteps RMS1 ON TREE.MaterialId = RMS1.FKRawMaterialId AND RMS1.ProcessingStepNo = 1 
     WHERE        1 = 1
GO
EXECUTE sp_addextendedproperty @name = N'MS_DiagramPaneCount', @value = 1, @level0type = N'SCHEMA', @level0name = N'dbo', @level1type = N'VIEW', @level1name = N'V_MaterialGenealogyDown';
...
Begin DesignProperties = 
   ...
   End
End
', @level0type = N'SCHEMA', @level0name = N'dbo', @level1type = N'VIEW', @level1name = N'V_MaterialGenealogyDown';
